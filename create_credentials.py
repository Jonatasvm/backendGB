...
# A chave privada COM as quebras de linha reais (não \n como texto)
...
with open(output_path, 'r') as f:
    check = json.load(f)
    key_lines = check['private_key'].split('\n')
    print(f"✓ Chave privada tem {len(key_lines)} linhas")
    print(f"✓ Primeira linha: {key_lines[0]}")
    print(f"✓ Última linha: {key_lines[-1]}")
